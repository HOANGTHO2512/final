...
export default function CareerFitProPage() {
  ...
  useEffect(() => {
    ...
    if (token) {
      setIsLoggedIn(true);
      setUsername(typeof window !== 'undefined' ? localStorage.getItem('cf_user') || 'Guest' : 'Guest');
    }
  }, []);
  ...
  // Toggle handlers
  const toggleCourse = (courseName: string, checked: boolean) => {
  ...
  const calculate = () => {
    ...
    const totalReq = courses.length;
    const reqChecked = selectedCourses.length;

    const scores: Record<string, number> = {};

    // Academic: course completion
    let acadBase = totalReq > 0 ? (reqChecked / totalReq) * 80 : 0;
    scores.academic = Math.min(100, Math.round(acadBase));
    ...
    // Match
    const targetTraits = DEPT_TRAITS[dept as keyof typeof DEPT_TRAITS]?.holland || [];
    const overlap = selectedHolland.filter(h => targetTraits.includes(h)).length;
    scores.match = overlap >= 2 ? 95 : (overlap === 1 ? 70 : 45);

    // Soft
    scores.soft = (selectedExp.some((e: any) => ['社團', '社團幹部', '競賽'].includes(e))) ? 90 : 60;
    ...
    if (scores.academic > 80) strengths.push('課程完成度高，學術基礎穩固');
    else weaknesses.push('課程學分有缺口，請優先確認修課計畫');
    ...
    setResults({
      total,
      scores,
      deptInfo: DEPT_TRAITS[dept as keyof typeof DEPT_TRAITS],
  ...
  // Reset
  const reset = () => {
    if (typeof window !== 'undefined' && confirm('重置將清除所有暫存資料，確定嗎？')) {
      setSelectedCourses([]);
      setSelectedExp([]);
      setSelectedCert([]);
      setSelectedHolland([]);
                    ...
                    <div className="flex flex-wrap gap-2">
                      {getCourses().map((course: any) => (
                        <Chip
                          ...
                          checked={selectedCourses.includes(typeof course === 'string' ? course : course.name)}
